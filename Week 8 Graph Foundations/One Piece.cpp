...
map<pp, int> neighbour(pp node)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (check(x, y))
        {
            if (g[node.first][node.second] == 1 && x == node.first && y == node.second + 1)
            {
                ans[{x, y}] = 0;
            }
            else if (g[node.first][node.second] == 2 && x == node.first && y == node.second - 1)
            {
                ans[{x, y}] = 0;
            }
            else if (g[node.first][node.second] == 3 && x == node.first + 1 && y == node.second)
            {
                ans[{x, y}] = 0;
            }
            else if (g[node.first][node.second] == 4 && x == node.first - 1 && y == node.second)
            {
                ans[{x, y}] = 0;
            }
            else
